...
function parseLatLng(club) {
  const [lat, lng, _] = club.lat_lng.replace("@", "").split(",")
    ...
    {clubs.filter(club => Array.isArray(club.lat_lng)).map((club) => (
      <Marker key={club.id} position={club.lat_lng} title={club.title}>
        <Popup>
          ...
          <h3>{club.title}</h3>
          {club.about}
          <div className="tags">{club.tags.map((t, idx) => <span className={`tag ${selectedTags.indexOf(t) > -1 ? "selected" : ""}`} key={idx}>{t}</span>)}</div>
          </div>
        </Popup>
      </Marker>
    ))}
